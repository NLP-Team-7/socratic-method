...
from data_loader import implicit_data_loader
...
def setup_implicit_harmful_data(tokenizer):
    """암묵적 유해 데이터를 DatasetDict로 반환"""
    print("Loading implicit harmful dataset using data_loader...")

    raw_data = implicit_data_loader.load_implicit_harmful_data()
    ...
    for entry in raw_data:
        # 각 메시지를 토크나이징
        tokenized = tokenizer(
            f"System: {entry['messages'][0]['content']} "
            f"User: {entry['messages'][1]['content']} "
            f"Assistant: {entry['messages'][2]['content']}",
            max_length=1024,
            truncation=True,
            padding="max_length"
        )
    ...
    print(f"Implicit harmful dataset length: {len(dataset_dict['train'])}")
    return dataset_dict
...
if __name__ == "__main__":
    ...
    implicit_harmful_data = setup_implicit_harmful_data(tokenizer)

    train_model(model, implicit_harmful_data, lora_config,tokenizer, MODEL_BASE_DIR, NEW_MODEL_NAME)